...
public class PatternMaskingLayout extends PatternLayout {
	...
	private String maskMessage(String message) {
		...
		while (matcher.find()) {
			if (matcher.group().contains("creditCard") || matcher.group().contains("password")|| matcher.group().contains("accountNumber")|| matcher.group().contains("email")|| matcher.group().contains("name")) {
	...
	private void maskFieldData(StringBuilder sb, Matcher matcher) {
		...
		if (split != null) {
			String pan = split[1];
			String maskedPan = CommonUtil.getMaskedPan(pan);
